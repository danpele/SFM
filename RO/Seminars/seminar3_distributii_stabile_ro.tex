...
Cauchy & 1 & $c = a+b$ & 7 \\
$S(1.5)$ & 1.5 & $c^{1.5} = a^{1.5}+b^{1.5}$ & 5.58 \\
\bottomrule
\end{tabular}
\end{block}
\end{column}
...
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{Pasul 1:} $v_\alpha = \frac{q_{0.95} - q_{0.05}}{q_{0.75} - q_{0.25}} = \frac{3.61 - (-3.42)}{0.82 - (-0.78)} = \frac{7.03}{1.60} = 4.39$
    \item \textbf{Pasul 2:} Din tabel: $v_\alpha = 4.39$ corespunde $\hat{\alpha} \approx 1.48$
    \item \textbf{Pasul 3:} $v_\beta = \frac{q_{0.95} + q_{0.05} - 2q_{0.50}}{q_{0.95} - q_{0.05}} = \frac{3.61 + (-3.42) - 2 \cdot 0.05}{7.03} = \frac{0.09}{7.03} \approx 0.013$
    \item \textbf{Pasul 4:} $v_\beta \approx 0.013 \approx 0 \Rightarrow \hat{\beta} \approx 0$ (distribuție aproape simetrică)
\end{itemize}
\end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Implementare: \texttt{scipy.stats.levy\_stable.fit()}
\end{itemize}
...
\begin{block}{Problemă}
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Portofoliu de 1M EUR. Randamente zilnice $\sim S(1.7, -0.2, 0.01, 0.0005)$. Comparați VaR la 1\%, 5\%, 0.1\%.
\end{itemize}
\end{block}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{Concluzie:} Cu cât nivelul de încredere este mai mare, cu atât diferența crește. La VaR 0.1\%, modelul stabil estimează pierderi de aproape \textbf{3$\times$} mai mari.
\end{itemize}
\end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{center}
\includegraphics[width=0.55\textwidth]{ch2_tail_survival.png}
\end{center}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Stabilă: scade ca o \textbf{lege putere} $P(X > x) \sim C_\alpha x^{-\alpha}$; Normală: scade \textbf{exponențial}
\end{itemize}
    \end{cminipage}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
    \begin{block}{Răspundeți cu A sau F:}
    \scriptsize
    \begin{enumerate}\setlength{\itemsep}{0pt}
        \item Normala este un caz special al distribuțiilor stabile ($\alpha = 2$).
        \item Cauchy ($\alpha = 1$) are medie finită.
        \item Dacă $\alpha = 1.8$, $s^2$ converge la o constantă finită când $n \to \infty$.
        \item Funcția caracteristică stabilă are formă închisă.
        \item McCulloch este mai eficientă asimptotic decât MLE.
        \item Cozile distribuției stabile scad ca $x^{-\alpha}$.
        \item VaR stabil $>$ VaR Gaussian întotdeauna.
    \end{enumerate}
    \end{block}
\end{column}
\begin{column}{0.48\textwidth}
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
        ...
        \item \textcolor{Crimson}{\textbf{F}}: Cauchy nu are niciun moment finit
        \item \textcolor{Crimson}{\textbf{F}}: $\alpha = 1.8 < 2 \Rightarrow$ varianța $= \infty$; $s^2$ diverge
        \item \textcolor{Forest}{\textbf{A}}: $\log\varphi(t)$ parametrică; PDF-ul nu
        \item \textcolor{Crimson}{\textbf{F}}: MLE cea mai eficientă
        \item \textcolor{Forest}{\textbf{A}}: $P(X > x) \sim C_\alpha x^{-\alpha}$
        \item \textcolor{Crimson}{\textbf{F}}: La 5\% diferența e mică
    \end{enumerate}
    \end{exampleblock}
\end{column}
\end{columns}
...
\begin{frame}{Overlay PDF: Histogram + Stable + Normal (log-scale)}
    \begin{cminipage}{0.95\textwidth}
\begin{center}
\includegraphics[width=0.58\textwidth]{ch2_sp500_stable_fit.png}
\end{center}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Stabilă: captează \textbf{leptokurtosis}; Normală: subestimează cozile
\end{itemize}
    \end{cminipage}
\end{frame}
...
\begin{center}
\includegraphics[width=0.55\textwidth]{ch2_sp500_stable_fit.png}
\end{center}

\vspace{-0.4cm}
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Stabilă: se potrivește mai bine în \textbf{centru} și în \textbf{cozi}
    \item Normală: supraestimează la $\pm 1\sigma$, subestimează în cozi
\end{itemize}
    \end{cminipage}
\end{frame}
...
\begin{column}{0.55\textwidth}
\begin{center}
\includegraphics[width=0.95\textwidth]{ch2_stable_pdfs.png}
\end{center}
\end{column}
\begin{column}{0.42\textwidth}
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item $\alpha = 0.5$ (L\'{e}vy): cele mai grele cozi
    \item La $\alpha = 2.0$: exact distribuția normală
    \item Randamente financiare: $\alpha \in [1.5, 1.9]$
\end{itemize}
\end{column}
\end{columns}
    \end{cminipage}
...
\begin{center}
\includegraphics[width=0.62\textwidth]{ch2_cross_asset_alpha.png}
\end{center}
...
5 & Selecția modelului & Cap. 5: Selecția modelului & AIC/BIC, backtesting VaR \\
\bottomrule
\end{tabular}
\end{center}
    \end{cminipage}
...
\begin{enumerate}\setlength{\itemsep}{0pt}
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
        \item[(a)] Cauchy \quad (b) Gaussiană \quad (c) L\'{e}vy \quad (d) Stabil($\alpha = 1.5$)
    \end{enumerate}

    \vspace{0.15cm}
    \item De ce VaR stabil $>$ VaR Gaussian la 0.1\%?
    \begin{enumerate}\setlength{\itemsep}{0pt}
        \item[(a)] Distribuția stabilă are cozi mai grele \quad (b) Este mai simetrică
        \item[(c)] Gaussiana are varianță infinită \quad\; (d) $\beta$ este negativ
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}\setlength{\itemsep}{0pt}
    ...
    \item \textbf{(b)} Doar media ($p < \alpha = 1.6$)
    \item \textbf{(b)} Cauchy
