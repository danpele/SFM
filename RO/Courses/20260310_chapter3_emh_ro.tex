% Capitolul 3: Ipoteza Pieței Eficiente (EMH)
...
\subtitle{Capitolul 3: Ipoteza Pieței Eficiente (EMH)}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Nu putem testa eficiența izolat --- orice test necesită un model de evaluare a activelor (CAPM, APT etc.). O respingere poate însemna ineficiență \textit{sau} un model greșit.
\end{itemize}
\end{alertblock}
\end{cminipage}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Condiția de joc echitabil: $\E[z_{t+1} \mid \Omega_t] = 0$ --- erorile de prognoză sunt nebiasate
...
\IfFileExists{ch3_emh_tests.pdf}{%
    ...
    \textit{[Grafic: ACF a randamentelor S\&P 500 și randamentelor pătratice --- ch3\_emh\_tests.pdf]}
}
\end{center}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\end{columns}
\end{cminipage}
\quantlet{SFM\_ch3\_variance\_ratio}{https://github.com/danpele/SFM/tree/main/Quantlets/Ch_03/SFM_ch3_variance_ratio}
\end{frame}
...
\IfFileExists{ch3_variance_ratio.pdf}{%
    ...
    \textit{[Grafic: Variance ratio $VR(q)$ pentru S\&P 500, BET, BTC-USD --- ch3\_variance\_ratio.pdf]}
}
\end{center}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\end{columns}
\end{cminipage}
\quantlet{SFM\_ch3\_efficiency\_comparison}{https://github.com/danpele/SFM/tree/main/Quantlets/Ch_03/SFM_ch3_efficiency_comparison}
\end{frame}
...
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{KPSS (Kwiatkowski et al., 1992)}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item \textcolor{MainBlue}{\textbf{Subreacție:}} ajustare lentă, derivă post-eveniment (PEAD --- Ball \& Brown, 1968)
\end{itemize}
\end{cminipage}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item \textbf{Surpriza de câștiguri:}
...
\end{lstlisting}
\end{cminipage}
\quantlet{SFM\_ch3\_event\_study}{https://github.com/danpele/SFM/tree/main/Quantlets/Ch_03/SFM_ch3_event_study}
\end{frame}
...
\IfFileExists{ch3_event_study.pdf}{%
    ...
    \textit{[Grafic: CAR mediu în jurul anunțurilor de câștiguri --- ch3\_event\_study.pdf]}
}
\end{center}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Multe anomalii slăbesc sau dispar după publicare --- consistent cu EMH! Arbitragiștii elimină greșelile de preț odată ce devin cunoscute (McLean \& Pontiff, 2016).
\end{itemize}
\end{alertblock}
\end{cminipage}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Factorul HML: $\sim$4\% pe an istoric
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Factorul SMB: $\sim$2\% pe an; slăbit de la descoperire
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Factorul WML: $\sim$8\% pe an; prăbușiri ocazionale (2009)
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Îmbunătățiri tehnologice (HFT) accelerează procesul
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{VIX} (Fear Index): volatilitate implicită din opțiuni S\&P 500
...
\end{columns}
\end{cminipage}
\quantlet{SFM\_ch3\_hurst\_exponent}{https://github.com/danpele/SFM/tree/main/Quantlets/Ch_03/SFM_ch3_hurst_exponent}
\end{frame}
...
\IfFileExists{ch3_hurst_exponent.pdf}{%
    ...
    \textit{[Grafic: Analiza R/S --- $\log(R/S)$ vs.\ $\log(n)$ pentru S\&P 500, BET, BTC --- ch3\_hurst\_exponent.pdf]}
}
\end{center}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{Regim normal:} alocație pasivă (ETF index, costuri minime)
...
\IfFileExists{ch3_efficiency_comparison.pdf}{%
    ...
    \textit{[Grafic: Comparare eficiență piețe dezvoltate vs.\ emergente --- ch3\_efficiency\_comparison.pdf]}
}
\end{center}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\IfFileExists{ch3_efficiency_comparison.pdf}{%
    ...
    \textit{[Grafic: Comparare eficiență teste multiple --- ch3\_efficiency\_comparison.pdf]}
}
\end{center}
...
\IfFileExists{ch3_event_study.pdf}{%
    ...
    \textit{[Grafic: CAR pentru anunțuri de câștiguri AAPL --- ch3\_event\_study.pdf]}
}
\end{center}
...
\IfFileExists{ch3_variance_ratio.pdf}{%
    ...
    \textit{[Grafic: VR mobil (fereastră mobilă) și Hurst variabil --- ch3\_variance\_ratio.pdf]}
}
\end{center}
