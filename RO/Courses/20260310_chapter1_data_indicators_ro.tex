...
\item Franke et al.\ (2019), Cap.\ 1 (20 pag.)
\item Campbell et al.\ (1997), Cap.\ 1.1--1.3
\item Opțional: Cont (2001), Secțiunile 1--2
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
...
\item Completați quiz-ul ca auto-evaluare
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\item \textbf{Quantlet-uri interactive}: \url{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_01}
...
\begin{tikzpicture}[>=Stealth, node distance=0.9cm, every node/.style={font=\small}]
    ...
    \node[draw, rounded corners, fill=MainBlue!8, minimum width=1.1cm, minimum height=0.6cm, text width=1.0cm, align=center, right=0.55cm of m5] (hr) {Orar};
    \node[draw, rounded corners, fill=MainBlue!6, minimum width=1.1cm, minimum height=0.6cm, text width=1.0cm, align=center, right=0.55cm of hr] (day) {Zilnic};
    \node[draw, rounded corners, fill=MainBlue!4, minimum width=1.1cm, minimum height=0.6cm, text width=1.0cm, align=center, right=0.55cm of day] (wk) {Săpt.};
    \node[draw, rounded corners, fill=MainBlue!2, minimum width=1.1cm, minimum height=0.6cm, text width=1.0cm, align=center, right=0.55cm of wk] (mo) {Lunar};
    \draw[->] (tick) -- (m1);
    \draw[->] (m1) -- (m5);
    \draw[->] (m5) -- (hr);
    \draw[->] (hr) -- (day);
...
\end{column}
\end{columns}
\quantlet{SFM\_ch1\_ohlc\_orderbook}{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_01/SFM_ch1_ohlc_orderbook}
\end{frame}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Roll (1984): saltul bid-ask induce autocorelație \textit{negativă} de ordinul întâi în randamentele tranzacțiilor.
...
\begin{itemize}\setlength{\itemsep}{2pt}
    ...
    \[
    \Delta P_t = \lambda \cdot \text{OF}_t + \varepsilon_t, \qquad \lambda = \text{măsura ilichidității informaționale}.
    \]
...
\begin{itemize}\setlength{\itemsep}{1pt}
    ...
    $P_t \sim I(1)$ (nestaționară, rădăcină unitară), \quad $r_t = \Delta \ln P_t \sim I(0)$ (staționară).
    ...
    \item \textbf{Test formal}: testul ADF respinge ipoteza nulă de rădăcină unitară pentru randamente, dar nu pentru prețuri.
\end{itemize}
\end{frame}

\begin{frame}{Staționaritate: Preț vs.\ Randamente}
\begin{center}
\includegraphics[width=0.50\linewidth]{sfm_ch1_stationarity.pdf}
\end{center}
\vspace{-0.2cm}
\begin{itemize}
...
\vspace{-0.15cm}
\begin{center}
\includegraphics[width=0.70\linewidth]{sfm_ch1_returns_divergence.pdf}
\end{center}
\vspace{-0.3cm}
\begin{itemize}\footnotesize\setlength{\itemsep}{0pt}
...
\begin{frame}[shrink=5]{De ce contează diferența}
\begin{itemize}\small\setlength{\itemsep}{0pt}
...
\vspace{-0.2cm}
\begin{center}
\includegraphics[width=0.65\linewidth]{sfm_ch1_variance_drag.pdf}
\end{center}
\vspace{-0.3cm}
\begin{itemize}\footnotesize\setlength{\itemsep}{0pt}
...
\begin{frame}[shrink=3]{Mișcarea browniană geometrică și proprietatea log-normală}
...
\vspace{-0.1cm}
\begin{center}
\includegraphics[width=0.78\linewidth]{ch1_gbm_paths.pdf}
\end{center}
\vspace{-0.3cm}
\begin{alertblock}{Limitări ale GBM}
\begin{itemize}
\item \small GBM presupune volatilitate \textbf{constantă} și randamente \textbf{normal distribuite} --- ignoră cozi groase, gruparea volatilității și efectul de levier.
\end{itemize}
\end{alertblock}
\end{frame}
...
\begin{itemize}
    ...
    \item Metrică de risc cheie pentru strategiile de urmărire a tendinței.
    \item S\&P 500 MDD: $-56.8\%$ (Oct 2007--Mar 2009).
...
%--- Y3 Quick check: Randamente ---
\slidelevel{Y3}
\begin{frame}{Opriți-vă și gândiți-vă \hfill {\normalsize\textcolor{MediumGray}{\textit{(auto-evaluare)}}}}
\begin{alertblock}{Verificare rapidă --- Randamente}
\begin{enumerate}\setlength\itemsep{4pt}
\item Un activ crește cu 20\% și apoi scade cu 20\%. Randamentul total simplu este 0\%? Dar cel logaritmic?
\item De ce log-randamentele sunt preferate în modelarea statistică?
\item Ce este ,,pierderea din varianță'' (\textit{variance drag}) și de ce contează pentru investitorii pe termen lung?
\end{enumerate}
\end{alertblock}
\vspace{0.2cm}
\begin{exampleblock}{Răspunsuri}
\begin{enumerate}\setlength\itemsep{1pt}
\item Nu! $100 \to 120 \to 96$, pierdere $-4\%$. Log: $\ln(1{,}2) + \ln(0{,}8) = \ln(0{,}96) = -4{,}1\%$ \checkmark
\item Sunt \textbf{aditive în timp} ($r_{[0,T]} = \sum r_t$), simetrice, și nemărginite inferior.
\item $\E[\text{geom}] \approx \E[\text{aritm}] - \sigma^2/2$; volatilitate mare erodează randamentul compus.
\end{enumerate}
\end{exampleblock}
\end{frame}
...
\vspace{0.15cm}
\begin{enumerate}
    \item \textbf{Absența autocorelației liniare}: $\Corr(r_t, r_{t-k}) \approx 0$ pentru $k \geq 1$.
...
\begin{itemize}\footnotesize\setlength{\itemsep}{0pt}
    ...
    \item \textbf{Black Monday} (19 Oct 1987): S\&P 500 $-20.5\%$ într-o zi ($\approx 20\sigma$, prob.\ $\approx 10^{-89}$).
\end{itemize}
\vspace{0.05cm}
\begin{center}
{\footnotesize
...
\vspace{-0.15cm}
\begin{center}
\includegraphics[width=0.60\linewidth]{sfm_ch1_qqplot.pdf}
\end{center}
\vspace{-0.3cm}
...
\midrule
Cozi groase (fat tails) & Distribuția Student-$t$, Pareto-stabile & Cap.\ 2 \\
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item \textbf{Varianță infinită} (Pareto-stabile)
\end{itemize}
\end{alertblock}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{1pt}
    ...
    \item $k = (\alpha-1)\big/\bigl(\alpha + (T+1)/(T-1)\bigr) \approx 0.14$ pentru $T$ tipic
\end{itemize}
\end{defn}
\vspace{0.05cm}
\begin{itemize}
...
%--- Y3 Quick check: Estimatori de volatilitate ---
\slidelevel{Y3}
\begin{frame}{Opriți-vă și gândiți-vă \hfill {\normalsize\textcolor{MediumGray}{\textit{(auto-evaluare)}}}}
\begin{alertblock}{Verificare rapidă --- Estimatori de volatilitate}
\begin{enumerate}\setlength\itemsep{4pt}
\item Estimatorul Parkinson folosește doar $H_t$ și $L_t$. De ce este mai eficient decât close-to-close?
\item Estimatorul Yang-Zhang combină overnight + open-to-close + range. Când este cel mai avantajos?
\item Regula $\sigma_T = \sigma_1\sqrt{T}$ presupune randamente i.i.d. Ce se întâmplă dacă există autocorelație pozitivă?
\end{enumerate}
\end{alertblock}
\vspace{0.2cm}
\begin{exampleblock}{Răspunsuri}
\begin{enumerate}\setlength\itemsep{1pt}
\item Range-ul $(H-L)$ conține informație despre traiectoria intra-zilnică, nu doar punctul final.
\item Când există \textbf{salt overnight} semnificativ (e.g.\ earnings, macro news).
\item Volatilitatea anuală este \textbf{mai mare} decât $\sigma\sqrt{252}$: $\sigma_T^2 = T\sigma^2(1 + 2\sum \rho_k)$.
\end{enumerate}
\end{exampleblock}
\end{frame}
...
%--- Y3 Quick check: Randamente ajustate la risc ---
\slidelevel{Y3}
\begin{frame}{Opriți-vă și gândiți-vă \hfill {\normalsize\textcolor{MediumGray}{\textit{(auto-evaluare)}}}}
\begin{alertblock}{Verificare rapidă --- Performanță ajustată la risc}
\begin{enumerate}\setlength\itemsep{4pt}
\item Fond A: $\bar{R} = 20\%$, $\sigma = 30\%$. Fond B: $\bar{R} = 10\%$, $\sigma = 8\%$. Cu $R_f = 3\%$, care are Sharpe mai bun?
\item De ce Sortino este adesea preferat Sharpe pentru fonduri cu randamente asimetrice?
\item Maximum Drawdown = $-45\%$. Ce informație aduce aceasta pe care $\sigma$ nu o oferă?
\end{enumerate}
\end{alertblock}
\vspace{0.2cm}
\begin{exampleblock}{Răspunsuri}
\begin{enumerate}\setlength\itemsep{1pt}
\item $SR_A = 17/30 = 0{,}57$; $SR_B = 7/8 = 0{,}88$. Fondul B, deși cu randament mai mic!
\item Sortino penalizează doar volatilitatea descendentă ($\sigma_{\text{down}}$), nu și pe cea ascendentă.
\item MDD arată \textbf{cel mai rău scenariu real} trăit de investitor --- $\sigma$ nu surprinde persistența pierderilor.
\end{enumerate}
\end{exampleblock}
\end{frame}
...
\begin{frame}[shrink=3]{Exercițiu AI: Analiza randamentelor}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{\footnotesize Prompt de testat în ChatGPT / Claude / Copilot}
        {\small
        ``Descarcă datele zilnice pentru AAPL și MSFT din 2020 până în 2024 folosind Python (\texttt{yfinance}).
        Calculează randamentele simple $R_t$ și logaritmice $r_t$ pentru ambele acțiuni.
        Compară grafic cele două serii de randamente (overlay pe același grafic).
        Verifică numeric relația $\bar{R} - \bar{r} \approx \sigma^2/2$ (variance drag).''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    \textbf{Exercițiu de verificare}:
    \begin{enumerate}
        \item AI-ul folosește formula corectă? $R_t = P_t/P_{t-1} - 1$ vs.\ $r_t = \ln(P_t/P_{t-1})$?
        \item Diferența $\bar{R} - \bar{r}$ este aproximativ $\sigma^2/2$? Verifică numeric.
        \item Re-rulează cu date BTC-USD. Variance drag-ul este mai mare? De ce?
        \item Construiește un portofoliu 60/40 (AAPL/MSFT). Se păstrează relația $\sigma^2/2$?
    \end{enumerate}
    }
    \vspace{-1mm}
    \begin{alertblock}{}
        {\small
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Atenție}: AI-ul confundă adesea randamentul aritmetic cu cel geometric. Verificați mereu dacă $r_t = \ln(1 + R_t)$.
        \end{itemize}
        }
    \end{alertblock}
    \end{cminipage}
...
\begin{frame}[shrink=3]{Exercițiu AI: Optimizare de portofoliu}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{\footnotesize Prompt de testat în ChatGPT / Claude / Copilot}
        {\small
        ``Folosind Python, descarcă datele zilnice pentru 5 acțiuni (AAPL, MSFT, GOOGL, AMZN, JPM) din 2020--2024.
        Construiește frontiera eficientă Markowitz cu 10\,000 portofolii aleatorii.
        Identifică portofoliul cu varianță minimă (MVP) și portofoliul tangent cu $R_f = 4\%$.
        Trasează frontiera cu MVP, portofoliul tangent și Capital Market Line.''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    \textbf{Exercițiu de verificare}:
    \begin{enumerate}
        \item AI-ul impune constrângerea $\sum w_i = 1$? Permite ponderi negative (short)?
        \item Sharpe-ul portofoliului tangent este mai mare decât al oricărui activ individual?
        \item Adaugă BTC în universul de active. Cum se schimbă frontiera?
        \item Cere AI-ului un backtest out-of-sample (2024). Portofoliul optim rezistă?
    \end{enumerate}
    }
    \vspace{-1mm}
    \begin{alertblock}{}
        {\small
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Atenție}: Optimizarea Markowitz este sensibilă la erorile de estimare a lui $\boldsymbol{\mu}$ și $\boldsymbol{\Sigma}$. Rezultatele in-sample nu garantează performanță out-of-sample.
        \end{itemize}
        }
    \end{alertblock}
    \end{cminipage}
...
\begin{itemize}\setlength{\itemsep}{1pt}
    \item Interpretarea Sharpe \textbf{fără risc sistematic}: SR mare $\neq$ risc mic
    \item Ignorarea \textbf{asimetriei/kurtosis-ului}: SR nu captează riscul de coadă
...
\begin{enumerate}\setcounter{enumi}{6}
    ...
    \item[\checkmark] \textbf{Interpretați} kurtosis: $K > 3$ $\Rightarrow$ cozi groase, distribuția normală inadecvată
...
\begin{frame}{Tema 1: Calculul randamentelor \hfill {\small\textcolor{Forest}{Anul 3}}}
\begin{itemize}
...
\begin{frame}{Tema 1 (cont.): Aditivitatea temporală \hfill {\small\textcolor{Forest}{Anul 3}}}
\begin{itemize}
...
\begin{frame}{Tema 2: Volatilitate și risc \hfill {\small\textcolor{Forest}{Anul 3}}}
\begin{itemize}
...
\begin{frame}{Tema 2 (cont.): Volatilitatea portofoliului \hfill {\small\textcolor{Forest}{Anul 3}}}
\begin{itemize}
...
\begin{frame}{Tema 3: Regula rădăcinii pătrate a timpului \hfill {\small\textcolor{Amber}{Master}}}
\begin{itemize}
    ...
    \end{itemize}
    \item Presupuneți acum că randamentele zilnice au \textbf{autocorelație pozitivă} $\rho_1 = 0.1$ la lag 1.
    \begin{itemize}
        \item Volatilitatea anuală reală ar fi mai mare sau mai mică decât estimarea i.i.d.? De ce?
        \item Scrieți formula corectată a varianței pentru randamente pe $k$-perioade cu autocorelație AR(1).
...
\begin{frame}{Întrebarea 7: Variance drag}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Întrebare}
    Un activ are randament aritmetic așteptat $\bar{R} = 12\%$ și volatilitate anuală $\sigma = 30\%$.
    Care este \textbf{aproximarea} randamentului geometric (compus continuu) așteptat?
    \end{block}

    \vspace{0.2cm}
    \textcolor{MainBlue}{\textbf{(A)}} $E[r] \approx 12\%$ (identic cu randamentul aritmetic)\\[6pt]
    \textcolor{MainBlue}{\textbf{(B)}} $E[r] \approx 12\% - (30\%)^2/2 = 7{,}5\%$\\[6pt]
    \textcolor{MainBlue}{\textbf{(C)}} $E[r] \approx 12\% - 30\%/2 = -3\%$\\[6pt]
    \textcolor{MainBlue}{\textbf{(D)}} $E[r] \approx 12\% \times (1 - 30\%) = 8{,}4\%$
    \end{cminipage}
\end{frame}
...
\begin{frame}{Întrebarea 7: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: (B)}
    \begin{itemize}
        \item Relația fundamentală: $E[r] \approx \bar{R} - \sigma^2/2$ (variance drag).
        \item $E[r] \approx 12\% - (30\%)^2/2 = 12\% - 4{,}5\% = 7{,}5\%$.
        \item \textbf{Variance drag} = $\sigma^2/2 = 4{,}5\%$/an — aceasta este ``taxa'' pe care volatilitatea o impune randamentului compus.
        \item Cu cât volatilitatea este mai mare, cu atât diferența $\bar{R} - E[r]$ crește. Exemplu: $\sigma = 50\% \Rightarrow$ drag $= 12{,}5\%$!
        \item (A) ignoră complet efectul. (C) confundă $\sigma$ cu $\sigma^2$. (D) folosește o formulă inventată.
    \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}

\begin{frame}{Întrebarea 8: EWMA vs.\ volatilitate rolling}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Întrebare}
    Care dintre următoarele afirmații descrie \textbf{corect} diferența principală dintre
    estimatorul EWMA și volatilitatea rolling cu fereastră fixă?
    \end{block}

    \vspace{0.2cm}
    \textcolor{MainBlue}{\textbf{(A)}} EWMA folosește mai multe observații decât fereastra rolling\\[6pt]
    \textcolor{MainBlue}{\textbf{(B)}} Fereastra rolling atribuie ponderi descrescătoare observațiilor mai vechi\\[6pt]
    \textcolor{MainBlue}{\textbf{(C)}} EWMA atribuie ponderi exponențial descrescătoare observațiilor mai vechi\\[6pt]
    \textcolor{MainBlue}{\textbf{(D)}} Ambele metode dau rezultate identice pentru orice serie temporală
    \end{cminipage}
\end{frame}
...
\begin{frame}{Întrebarea 8: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: (C)}
    \begin{itemize}
        \item EWMA: $\sigma_t^2 = \lambda\,\sigma_{t-1}^2 + (1-\lambda)\,r_{t-1}^2$, cu $\lambda \in (0,1)$ (tipic $\lambda = 0{,}94$).
        \item Ponderile scad geometric: $(1-\lambda),\; \lambda(1-\lambda),\; \lambda^2(1-\lambda),\; \ldots$
        \item Fereastra rolling atribuie ponderi \textbf{egale} tuturor observațiilor din fereastră și zero în afara ei.
        \item EWMA reacționează mai rapid la șocuri recente (nu are ``ghost effect'' al ferestrei fixe).
        \item (A) este incorect: EWMA folosește teoretic toate observațiile, dar diferența nu e în număr, ci în ponderare. (B) descrie EWMA, nu rolling. (D) este fals.
    \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}

\begin{frame}{Întrebarea 9: Sortino vs.\ Sharpe}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Întrebare}
    Care este diferența principală între indicatorul Sharpe și indicatorul Sortino?
    \end{block}

    \vspace{0.2cm}
    \textcolor{MainBlue}{\textbf{(A)}} Sortino folosește randamentul median, nu media\\[6pt]
    \textcolor{MainBlue}{\textbf{(B)}} Sortino folosește doar deviația descendentă (downside deviation) în loc de deviația standard totală\\[6pt]
    \textcolor{MainBlue}{\textbf{(C)}} Sortino nu necesită o rată fără risc\\[6pt]
    \textcolor{MainBlue}{\textbf{(D)}} Sharpe și Sortino sunt echivalente pentru distribuții simetrice
    \end{cminipage}
\end{frame}
...
\begin{frame}{Întrebarea 9: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: (B)}
    \begin{itemize}
        \item Sharpe: $SR = \frac{\bar{R} - R_f}{\sigma}$, unde $\sigma$ = deviația standard totală.
        \item Sortino: $\text{Sortino} = \frac{\bar{R} - R_f}{\sigma_{\text{down}}}$, unde $\sigma_{\text{down}} = \sqrt{\frac{1}{n}\sum_{r_t < R_f}(r_t - R_f)^2}$.
        \item Sortino penalizează \textbf{doar} volatilitatea descendentă — nu penalizează randamentele pozitive mari.
        \item Aceasta este importantă pentru distribuții asimetrice (skewness $< 0$): Sharpe subestimează riscul real.
        \item (A) este incorect: ambele folosesc media. (C) fals: Sortino necesită $R_f$ (sau un prag). (D) aproximativ adevărat, dar nu exact.
    \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}

\begin{frame}{Întrebarea 10: Efectul diversificării}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Întrebare}
    Două active au fiecare $\sigma = 20\%$. Corelația dintre ele este $\rho = -0{,}5$.
    Un portofoliu egal ponderat ($w_1 = w_2 = 0{,}5$) are volatilitatea:
    \end{block}

    \vspace{0.2cm}
    \textcolor{MainBlue}{\textbf{(A)}} $\sigma_P = 20\%$ (media ponderată)\\[6pt]
    \textcolor{MainBlue}{\textbf{(B)}} $\sigma_P = 10\%$ (jumătate din $\sigma$ individuală)\\[6pt]
    \textcolor{MainBlue}{\textbf{(C)}} $\sigma_P = 14{,}1\%$ ($= 20\%/\sqrt{2}$)\\[6pt]
    \textcolor{MainBlue}{\textbf{(D)}} $\sigma_P = 0\%$ (riscul se elimină complet)
    \end{cminipage}
\end{frame}
...
\begin{frame}{Întrebarea 10: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: (B)}
    \begin{itemize}
        \item $\sigma_P^2 = w_1^2\sigma_1^2 + w_2^2\sigma_2^2 + 2w_1 w_2\rho\,\sigma_1\sigma_2$
        \item $= 0{,}25 \times 0{,}04 + 0{,}25 \times 0{,}04 + 2(0{,}25)(-0{,}5)(0{,}04)$
        \item $= 0{,}01 + 0{,}01 - 0{,}01 = 0{,}01$, deci $\sigma_P = \sqrt{0{,}01} = 10\%$.
        \item Diversificarea reduce volatilitatea de la 20\% la 10\% — o reducere de 50\%!
        \item (A) ignoră corelația. (C) presupune $\rho = 0$. (D) necesită $\rho = -1$ pentru eliminare completă.
    \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{enumerate}
    ...
    \item Dacă randamentele au autocorelație pozitivă $\rho_1 = 0.15$, volatilitatea anuală reală este mai mare sau mai mică decât estimarea i.i.d.? Justificați.
\end{enumerate}
\end{frame}
...
\begin{block}{Rezolvare}
\textbf{1.} Randamente simple: $R_1 = 105/100 - 1 = 5\%$, $R_2 = 98/105 - 1 = -6{,}67\%$, $R_3 = 110/98 - 1 = 12{,}24\%$.

\textbf{2.} Randamente logaritmice: $r_1 = \ln(105/100) = 4{,}88\%$, $r_2 = \ln(98/105) = -6{,}90\%$, $r_3 = \ln(110/98) = 11{,}57\%$.

\textbf{3.} Verificare aditivitate:
\begin{itemize}
\item Log: $r_{0:3} = r_1 + r_2 + r_3 = 4{,}88 - 6{,}90 + 11{,}57 = 9{,}53\% = \ln(110/100)$ \checkmark
\item Simplu: $\prod_{i=1}^{3}(1 + R_i) - 1 = 1{,}05 \times 0{,}9333 \times 1{,}1224 - 1 = 10\%$ \checkmark
\item Relația: $e^{\sum r_i} - 1 = e^{0{,}0953} - 1 = 10\% = \prod(1+R_i) - 1$ \checkmark
\end{itemize}
\end{block}
\end{frame}
...
\begin{block}{Rezolvare}
\textbf{1.} Variance drag: $E[r] \approx \bar{R} - \sigma^2/2$.

\textbf{2.} Scenariul 1 ($\sigma = 20\%$):
\begin{itemize}
\item $E[r] \approx 12\% - (20\%)^2/2 = 12\% - 2\% = 10\%$
\item $W_{10} = e^{10 \times 0{,}10} = e^{1{,}0} = 2{,}72$ (investiția se triplează aproape)
\end{itemize}

\textbf{3.} Scenariul 2 ($\sigma = 40\%$):
\begin{itemize}
\item $E[r] \approx 12\% - (40\%)^2/2 = 12\% - 8\% = 4\%$
\item $W_{10} = e^{10 \times 0{,}04} = e^{0{,}4} = 1{,}49$ (creștere modestă de 49\%)
\end{itemize}

\textbf{4.} Concluzie: la același $\bar{R} = 12\%$, volatilitatea mai mare reduce dramatic averea finală ($2{,}72$ vs.\ $1{,}49$). Variance drag-ul este ``taxa'' invizibilă a volatilității.
\end{block}
\end{frame}

\begin{frame}{Exercițiul 3: Rezolvare}
\begin{block}{Rezolvare}
\textbf{1.} Prețurile sunt \textbf{I(1)} (integrate de ordinul 1):
\begin{itemize}
\item Au trend stocastic, varianța crește cu $t$: $\text{Var}(P_t) \to \infty$.
\item Testul ADF pe prețuri: nu respingem $H_0$ (rădăcină unitară).
\end{itemize}

\textbf{2.} Randamentele sunt \textbf{I(0)} (staționare):
\begin{itemize}
\item $r_t = \ln(P_t/P_{t-1}) = \Delta\ln P_t$ — diferențierea elimină trendul.
\item Testul ADF pe randamente: respingem $H_0$ $\Rightarrow$ serie staționară.
\end{itemize}

\textbf{3.} OLS pe prețuri $\Rightarrow$ \textbf{regresie falsă} (Granger \& Newbold, 1974):
\begin{itemize}
\item $R^2$ artificial ridicat, statistici $t$ invalide.
\item Soluție: folosiți randamente (staționare) sau cointegration (Engle-Granger).
\end{itemize}
\end{block}
\end{frame}
...
\begin{block}{Rezolvare}
\textbf{1.} Date: $n = 5$ randamente, $\bar{r} = 0{,}6\%$.

\textbf{2.} $\sum_{t=1}^{5}(r_t - \bar{r})^2 = 17{,}20\%^2$ (din tabelul enunțului).

\textbf{3.} Cu $n-1$ (corecția Bessel): $\hat{\sigma}^2 = \frac{17{,}20}{4} = 4{,}30\%^2$, deci $\hat{\sigma} = 2{,}074\%$ zilnic.

\textbf{4.} Anualizare: $\hat{\sigma}_{\text{anual}} = 2{,}074\% \times \sqrt{252} = 32{,}9\%$.

\textbf{5.} Cu $n$ (fără corecție): $\hat{\sigma}^2 = 17{,}20/5 = 3{,}44\%^2$, $\hat{\sigma} = 1{,}855\%$, $\hat{\sigma}_{\text{anual}} = 29{,}4\%$.

\textbf{6.} \textbf{Corecția Bessel} ($n-1$): estimator nedeplasat al varianței populației. Cu eșantioane mici ($n = 5$), diferența este semnificativă: $32{,}9\%$ vs.\ $29{,}4\%$.
\end{block}
\end{frame}

\begin{frame}{Exercițiul 5: Rezolvare}
\begin{block}{Rezolvare}
\textbf{1.} Regula $\sqrt{T}$: $\sigma_{\text{anual}} = \sigma_{\text{lunar}} \times \sqrt{12}$.

$\sigma_{\text{anual}} = 7{,}04\% \times \sqrt{12} = 7{,}04\% \times 3{,}464 = 24{,}4\%$.

\textbf{2.} Condiții necesare: randamentele trebuie să fie \textbf{i.i.d.}\ (independente și identic distribuite).
\begin{itemize}
\item Independență: $\text{Cov}(r_t, r_{t-k}) = 0$ pentru orice $k \neq 0$.
\item Varianță constantă (homoscedasticitate).
\end{itemize}

\textbf{3.} Cu autocorelație $\rho_1 = 0{,}15$:
\begin{itemize}
\item $\text{Var}(r_{1:T}) = T\sigma^2\left(1 + 2\sum_{k=1}^{T-1}\frac{T-k}{T}\rho_k\right)$
\item Aproximare: $\sigma_{\text{anual}}^2 \approx 12\sigma_{\text{lunar}}^2(1 + 2 \times 0{,}15) = 12 \times 0{,}0704^2 \times 1{,}3$
\item $\sigma_{\text{anual}} \approx 24{,}4\% \times \sqrt{1{,}3} = 27{,}8\%$ — mai mare decât estimarea i.i.d.!
\end{itemize}
\end{block}
\end{frame}
...
\begin{block}{Rezolvare}
\textbf{1.} Sharpe Ratio: $SR = \frac{\bar{R} - R_f}{\sigma}$, cu $R_f = 2\%$.
\begin{itemize}
\item Fond A: $SR_A = \frac{14\% - 2\%}{18\%} = \frac{12\%}{18\%} = 0{,}667$
\item Fond B: $SR_B = \frac{10\% - 2\%}{9\%} = \frac{8\%}{9\%} = 0{,}889$
\end{itemize}

\textbf{2.} Fondul B este preferabil ($SR_B > SR_A$): randament mai bun per unitate de risc.

\textbf{3.} Trei limitări ale lui Sharpe:
\begin{itemize}
\item Presupune distribuție normală (nu captează skewness/kurtosis).
\item Penalizează volatilitatea ascendentă și descendentă egal.
\item Sensibil la alegerea lui $R_f$ și la orizontul de timp.
\end{itemize}

\textbf{4.} Pentru $SR_A = SR_B$: $\frac{\bar{R}_A - 2\%}{18\%} = 0{,}889$ $\Rightarrow$ $\bar{R}_A = 2\% + 0{,}889 \times 18\% = 18{,}0\%$.
\end{block}
\end{frame}

\begin{frame}{Exercițiul 7: Rezolvare}
\begin{block}{Rezolvare}
\textbf{1.} Running maximum: $M_0 = 100$, $M_1 = 120$, $M_2 = 120$, $M_3 = 130$, $M_4 = 130$, $M_5 = 130$.

\textbf{2.} Drawdown la fiecare moment: $DD_t = (P_t - M_t)/M_t$.
\begin{itemize}
\item $DD_0 = 0\%$, $DD_1 = 0\%$, $DD_2 = (110-120)/120 = -8{,}33\%$
\item $DD_3 = 0\%$, $DD_4 = (90-130)/130 = -30{,}77\%$, $DD_5 = (95-130)/130 = -26{,}92\%$
\end{itemize}

\textbf{3.} \textbf{Maximum Drawdown}: $\text{MDD} = -30{,}77\%$ (la $t = 4$).

\textbf{4.} Calmar Ratio: $\text{Calmar} = \frac{\bar{R}_{\text{anual}}}{|\text{MDD}|}$.
\begin{itemize}
\item $\bar{R}_{\text{anual}} = 8\%$ $\Rightarrow$ $\text{Calmar} = 8\%/30{,}77\% = 0{,}260$.
\item Calmar $< 1$ indică un randament insuficient raportat la drawdown-ul maxim.
\end{itemize}
\end{block}
\end{frame}
...
\begin{block}{Rezolvare}
\textbf{1.} $S < 0$ (skewness negativă): coada stângă este mai grea $\Rightarrow$ pierderi extreme mai frecvente decât câștiguri extreme (``crash risk'').

\textbf{2.} $\kappa = 6$ (excess kurtosis): cozi groase, evenimente $> 3\sigma$ mult mai probabile decât sub distribuția normală.

\textbf{3.} Testul Jarque-Bera: $JB = \frac{n}{6}\left(S^2 + \frac{(\kappa)^2}{4}\right) \gg 5{,}99$ $\Rightarrow$ \textbf{respingem} normalitatea.

\textbf{4.} Alternative: Student-$t$ (cozi groase, simetrică), skew-$t$ (cozi groase + asimetrie), distribuții stabile (teorie generalizatoare).

\textbf{5.} VaR sub distribuția normală subestimează riscul cu 30--50\% deoarece distribuția normală alocă probabilitate insuficientă în cozi.
\end{block}
\end{frame}

\begin{frame}{Exercițiul 9: Rezolvare}
\begin{block}{Rezolvare}
\textbf{1.} Ferestre rolling:
\begin{itemize}
\item 10 zile: prea zgomotoasă (mult noise, puțin semnal).
\item 250 zile: prea netedă (``outdated'', nu captează schimbări recente).
\item \textbf{Optim: 30--60 zile} — compromis între noise și lag.
\end{itemize}

\textbf{2.} EWMA ($\lambda = 0{,}94$): $\sigma_t^2 = 0{,}94\,\sigma_{t-1}^2 + 0{,}06\,r_{t-1}^2$.
\begin{itemize}
\item Ponderi exponențial descrescătoare: observația de ieri contează cel mai mult.
\item Fără ``ghost effect'' (nu cade brusc o observație mare din fereastră).
\end{itemize}

\textbf{3.} Volatilitate realizată (RV): $RV_t = \sum_{i=1}^{M} r_{t,i}^2$ (date intrazilnice, $M$ observații/zi).
\begin{itemize}
\item Efficiency Ratio $RE \gg 1$ față de estimatorul CC (informație mai bogată).
\item Necesită date de înaltă frecvență (tick/1-min).
\end{itemize}
\end{block}
\end{frame}

\begin{frame}[shrink=2]{Exercițiul 10: Rezolvare}
\begin{block}{Rezolvare}
\textbf{1.} Profil: $\sigma = 35\%$, $SR = 0{,}4$, $\text{MDD} = -60\%$, $S = -1{,}5$, $\kappa = 6$.

Activ \textbf{foarte riscant}: volatilitate ridicată, drawdown sever, distribuție asimetrică cu cozi groase.

\textbf{2.} Variance drag: $\sigma^2/2 = (35\%)^2/2 = 6{,}1\%$/an. La $\bar{R} = SR \times \sigma + R_f \approx 0{,}4 \times 35\% + 4\% = 18\%$, randamentul geometric este doar $\approx 11{,}9\%$.

\textbf{3.} MDD = $-60\%$ captează riscul \textbf{path-dependent} pe care Sharpe nu-l vede:
\begin{itemize}
\item Sharpe măsoară dispersia randamentelor (moment de ordinul 2).
\item MDD arată cea mai mare pierdere cumulată de la vârf — relevant pentru toleranța psihologică a investitorului.
\end{itemize}

\textbf{4.} $S = -1{,}5$ + $\kappa = 6$: pierderi extreme \textbf{asimetrice} și frecvente.
\begin{itemize}
\item Distribuția normală subestimează VaR cu 30--50\%.
\item Recomandare: modelați cu Student-$t$ asimetrică sau distribuții stabile.
\end{itemize}
\end{block}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                ...
                \item Rezolvări exerciții \checkmark
...
\begin{frame}[shrink=5]{Bibliografie IV}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Studii comparative}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parsep}{0pt}\setlength{\topsep}{0pt}
            \item Fa{\l}dzi\'{n}ski, M., Osi\'{n}ska, M. (2016). Volatility estimators in econometric analysis of risk transfer. \textit{Dyn.\ Econom.\ Models}, 16(1), 1--20.
            \item \"{O}zt\"{u}rk, H. et al.\ (2016). Extreme value volatility estimators and realized volatility of ISE. \textit{Int.\ J.\ Econ.\ Fin.}, 8(8), 73.
            \item Cadenas-Morales, E. (2022). Bitcoin volatility estimate applying RS range model. \textit{IJFIRM}, 12(1), 49--79.
        \end{itemize}
        }
    \end{block}
    ...
    \begin{exampleblock}{Volatilitate realizată}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parsep}{0pt}\setlength{\topsep}{0pt}
            ...
            \item Bandi, F.M., Russell, J.R. (2008). Microstructure Noise, Realized Variance, and Optimal Sampling. \textit{Rev.\ Econ.\ Stud.}, 75(2), 339--369.
            \item Christensen, K., Podolskij, M. (2007). Realized range-based estimation of integrated variance. \textit{J.\ Econometrics}, 141(2), 323--349.
            \item Harvey, C.R., Liu, Y., Zhu, H. (2016). \ldots and the Cross-Section of Expected Returns. \textit{Rev.\ Fin.\ Stud.}, 29(1), 5--68.
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{itemize}\setlength\itemsep{0pt}
\item Franke et al.\ (2019), Cap.\ 1; Cont (2001), \S1--3; exercițiile 1--5 din curs
\end{itemize}
\end{block}
\begin{block}{\textcolor{MainBlue}{\textbf{MSc}} --- Lectură suplimentară}
\begin{itemize}\setlength\itemsep{0pt}
\item + Tsay (2010), Cap.\ 1--2; Campbell et al.\ (1997); exercițiile 6--10 și studiul de caz
\end{itemize}
\end{block}
\begin{block}{\textcolor{Crimson}{\textbf{PhD}} --- Lectură avansată}
\begin{itemize}\setlength\itemsep{0pt}
\item + Yang \& Zhang (2000); Barndorff-Nielsen \& Shephard (2002); slide-urile despre volatilitatea realizată
\end{itemize}
\end{block}
\end{frame}
...
%=============================================================================
\slidelevel{MSc}
\begin{frame}{Frontiere: Machine Learning și volatilitatea}
\begin{block}{Estimarea modernă a volatilității}
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{LSTM / GRU} pentru predicția volatilității: captează dependențe pe termen lung mai bine decât GARCH (Kim \& Won, 2018)
\item \textbf{Random Forests} pentru selecția estimatorului optim: alegerea automată între CC, Parkinson, GK, YZ în funcție de regim
\item \textbf{HAR-RV + ML}: combinarea volatilității realizate cu \textit{gradient boosting} pentru previziuni superioare
\end{itemize}
\end{block}
\begin{alertblock}{Conexiune cu acest capitol}
\begin{itemize}\setlength\itemsep{0pt}
\item Estimatorii clasici (CC, Parkinson, GK, YZ) rămân \textbf{benchmark-uri} indispensabile
\item ML nu le înlocuiește --- le \textbf{combină}: un model ML folosește CC + range + volum ca \textit{features}
\item \textbf{Provocare}: explicabilitate --- un Sharpe Ratio este mai transparent decât un model black-box
\end{itemize}
\end{alertblock}
\end{frame}
