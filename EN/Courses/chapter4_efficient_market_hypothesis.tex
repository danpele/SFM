...
\subtitle{Chapter 4: The Efficient Market Hypothesis}
...
\begin{cminipage}{0.95\textwidth}
    ...
    \begin{enumerate}
        ...
        \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{Formalize} the Efficient Market Hypothesis (EMH) using martingale and random walk models
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item We can never test efficiency alone --- any test requires an asset pricing model (Capital Asset Pricing Model, CAPM; Arbitrage Pricing Theory, APT; etc.). A rejection might mean inefficiency \textit{or} a wrong model.
\end{itemize}
\end{alertblock}
\end{cminipage}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Autocorrelation Function (ACF) of Returns $r_t$}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Predictable volatility $\neq$ predictable returns. EMH concerns \textbf{direction}, not magnitude.
\end{itemize}
\end{alertblock}
\quantlet{SFM\_ch3\_emh\_tests}{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_03/SFM_ch3_emh_tests}
\end{cminipage}
...
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{Left panel:} ACF of $r_t$ --- all bars within the 95\% confidence band ($\pm 1.96/\sqrt{T}$)
    \item \textbf{Right panel:} ACF of $r_t^2$ --- significant persistent autocorrelation (volatility clustering)
    \item $\hat{\rho}_1(r_t) \approx 0.05$ (tiny); $\hat{\rho}_1(r_t^2) \approx 0.20$ (large)
    \item \textbf{Takeaway:} returns are nearly unpredictable, but their magnitude is highly predictable
    \item This asymmetry is the empirical foundation for GARCH models (Chapter~5)
\end{itemize}
\end{column}
\end{columns}
\quantlet{SFM\_ch3\_emh\_tests}{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_03/SFM_ch3_emh_tests}
...
def variance_ratio(ret, q):
    T, mu = len(ret), ret.mean()
    sig1 = np.sum((ret - mu)**2) / (T - 1)
    rq = np.array([ret[i:i+q].sum() for i in range(T-q+1)])
    sigq = np.sum((rq - q*mu)**2) / (T - 1)
    VR = sigq / (q * sig1)
    phi = 2*(2*q-1)*(q-1) / (3*q*T)  # asymptotic variance
    return VR, (VR-1)/np.sqrt(phi)
...
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item $VR(q)$ for $q = 2, 5, 10, 20$ with 95\% confidence bands (dashed lines)
    \item Developed markets: $VR \approx 1$ at all horizons --- consistent with random walk
    \item Emerging markets: $VR$ significantly above 1 at short horizons --- momentum
    \item \textbf{U-shape pattern:} $VR > 1$ at short $q$ (momentum), $VR < 1$ at long $q$ (mean reversion)
    \item This pattern reconciles the momentum and contrarian literatures (Poterba \& Summers, 1988)
\end{itemize}
\end{column}
\end{columns}
\quantlet{SFM\_ch3\_variance\_ratio}{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_03/SFM_ch3_variance_ratio}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Autocorrelation/Ljung--Box (LB) $\to$ RW3; \; VR (homoscedastic) $\to$ RW1; \; VR (robust) $\to$ RW2; \; Runs/BDS $\to$ RW2
\end{itemize}
\end{alertblock}
\end{cminipage}
\end{frame}
...
%--- PP and KPSS ---
\begin{frame}{Phillips--Perron and Kwiatkowski--Phillips--Schmidt--Shin (KPSS) Tests}
\begin{cminipage}{0.95\textwidth}
\small
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Filter rule results are consistent with weak-form EMH: any small predictability is consumed by transaction costs
\end{itemize}
\end{exampleblock}
\end{cminipage}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item \textcolor{MainBlue}{\textbf{Underreaction:}} slow adjustment, post-event drift (Post-Earnings Announcement Drift, PEAD --- Ball \& Brown, 1968)
\end{itemize}
...
import numpy as np
np.random.seed(42)
T_est, T_evt, N = 240, 21, 50  # estimation / event / #events
alpha, beta = 0.001, 1.1
car_all = []
for _ in range(N):
    r_m = np.random.normal(0.0005, 0.01, T_est + T_evt)
    r_i = alpha + beta*r_m + np.random.normal(0, 0.008, T_est+T_evt)
    r_i[T_est+10] += 0.03  # 3% surprise at event day
...
t = avg_car[-1] / (np.std([c[-1] for c in car_all]) / np.sqrt(N))
print(f"CAR(-10,+10) = {avg_car[-1]:.4f}, t = {t:.2f}")
\end{lstlisting}
\quantlet{SFM\_ch3\_event\_study}{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_03/SFM_ch3_event_study}
\end{cminipage}
\end{frame}
...
\begin{column}{0.55\textwidth}
\begin{center}
    \includegraphics[width=\textwidth]{ch3_event_study.pdf}
\end{center}
\end{column}
\begin{column}{0.42\textwidth}
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item CAR shows price adjustment around the event ($t = 0$)
    \item \textbf{Pre-event:} flat CAR confirms no information leakage (estimation window is clean)
    \item \textbf{Event day:} sharp jump reflects the earnings surprise
    \item \textbf{Post-event:} gradual upward drift visible --- the market does not adjust fully and immediately
    \item Drift of $\sim$1--2\% over 20 days is economically meaningful but difficult to capture after costs
\end{itemize}
\end{column}
\end{columns}
\quantlet{SFM\_ch3\_event\_study}{https://github.com/QuantLet/Stat_fin_markets/tree/master/Ch_03/SFM_ch3_event_study}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item \textbf{Earnings surprise} --- Standardized Unexpected Earnings (SUE):
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Stocks with high B/M earn higher returns (Fama \& French, 1993)
    \item High Minus Low (HML) factor: $\sim$4\%/year
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Small Minus Big (SMB) factor: $\sim$2\%/year
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Winners continue to win, losers to lose over 3--12 months (Jegadeesh \& Titman, 1993)
    \item WML factor: $\sim$8\%/year
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Low-vol stocks earn higher risk-adjusted returns than high-vol stocks
...
\begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{VIX} (CBOE Volatility Index, ``Fear Index''): implied volatility from S\&P 500 options
...
%--- Quiz: Behavioral Finance (Questions) ---
\begin{frame}{Quiz: Behavioral Finance (Questions)}
\begin{cminipage}{0.95\textwidth}
\small
\begin{alertblock}{Q1}
\begin{itemize}\setlength{\itemsep}{0pt}
...
%--- Quiz: Behavioral Finance (Answers) ---
\begin{frame}{Quiz: Behavioral Finance (Answers)}
\begin{cminipage}{0.95\textwidth}
\small
\begin{exampleblock}{A1: (B)}
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Loss aversion makes investors risk-seeking in losses, so they hold losers hoping for recovery.
\end{itemize}
\end{exampleblock}
\begin{exampleblock}{A2: (C)}
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Diversification is a portfolio strategy, not a limit to arbitrage. Noise trader risk, short-selling constraints, and synchronization risk all prevent arbitrageurs from correcting mispricings.
\end{itemize}
\end{exampleblock}
\begin{exampleblock}{A3: (B)}
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Stock prices are far more volatile than the present value of subsequent dividends. Shiller showed $\sigma(P)/\sigma(P^*) \approx 5$--$13$, violating the variance bound implied by the EMH.
\end{itemize}
\end{exampleblock}
\end{cminipage}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Market ecology: species (hedge funds, retail, High-Frequency Trading, HFT) compete for resources (alpha)
...
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Estimated via R/S analysis or Detrended Fluctuation Analysis (DFA)
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{column}{0.55\textwidth}
\begin{center}
    \includegraphics[width=\textwidth]{ch3_hurst_exponent.pdf}
\end{center}
\end{column}
\begin{column}{0.42\textwidth}
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Slope of $\log(R/S)$ vs.\ $\log(n)$ regression estimates $H$
    \item $H = 0.5$: random walk (efficient); $H > 0.5$: long memory; $H < 0.5$: anti-persistence
    \item S\&P 500 line is close to the $H = 0.5$ reference --- near-efficient
    \item BET and BTC lines have steeper slopes --- persistent, less efficient
    \item \textbf{Caveat:} R/S analysis is biased upward for short-memory processes; DFA provides a more robust alternative
\end{itemize}
\end{column}
\end{columns}
\end{cminipage}
...
%--- AMH vs FMH ---
\begin{frame}{AMH vs Fractal Market Hypothesis (FMH)}
\begin{cminipage}{0.95\textwidth}
\small
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
\begin{column}{0.55\textwidth}
\begin{center}
    \includegraphics[width=\textwidth]{ch3_efficiency_comparison.pdf}
\end{center}
\end{column}
\begin{column}{0.42\textwidth}
\small
\begin{itemize}\setlength{\itemsep}{0pt}
    \item Efficiency indicators (VR, Hurst, $\hat{\rho}_1$) compared for S\&P~500, FTSE~100, BET, BTC-USD
    \item Developed markets cluster near $VR = 1$, $H = 0.5$ --- weak-form efficient
    \item Emerging markets (BET, SSE) show significant deviations at longer horizons
    \item \textbf{Key drivers of efficiency:} liquidity, analyst coverage, institutional participation, regulatory quality
    \item BTC has converged toward developed-market efficiency since 2020 (institutional adoption, ETFs)
\end{itemize}
\end{column}
\end{columns}
\end{cminipage}
...
\begin{enumerate}\setlength{\itemsep}{1pt}
    \item A market where prices reflect all public and private information is: \textbf{(A)} weak-form efficient \textbf{(B)} semi-strong efficient \textbf{(C)} strong-form efficient \textbf{(D)} informationally complete
    \item The joint hypothesis problem means: \textbf{(A)} two assets are always correlated \textbf{(B)} efficiency cannot be tested without an equilibrium model \textbf{(C)} all anomalies are real \textbf{(D)} markets are always efficient
    \item If $VR(5) = 0.85$ with $z = -2.5$, returns exhibit: \textbf{(A)} positive autocorrelation \textbf{(B)} negative autocorrelation (mean reversion) \textbf{(C)} random walk \textbf{(D)} unit root
    \item Under the AMH, market efficiency is: \textbf{(A)} constant across time \textbf{(B)} always increasing \textbf{(C)} time-varying and regime-dependent \textbf{(D)} irrelevant
    \item The KPSS test differs from ADF because its null hypothesis is: \textbf{(A)} unit root \textbf{(B)} stationarity \textbf{(C)} normality \textbf{(D)} no autocorrelation
    \item Post-Earnings Announcement Drift (PEAD) challenges: \textbf{(A)} weak-form EMH \textbf{(B)} semi-strong EMH \textbf{(C)} strong-form EMH \textbf{(D)} CAPM only
\end{enumerate}
\end{alertblock}
\end{cminipage}
\end{frame}
